      ...
      <ul className='flex flex-wrap gap-2 xl:gap-6'>
        {categoryData?.data.map((item: CategoryItem) => {
          ...
          return (
            <li
              key={item.id}
              className='hover-gadget-link h-[180px] rounded-2xl bg-card-repair-gradient md:w-[calc((100%-32px)/3)] xl:h-[261px] xl:w-[calc((100%-50px)/3)]'
            >
              <Link
                href={`/repair/${categoryPath}`}
                className='flex w-full flex-col justify-between rounded-2xl p-7 hover:bg-dark-blue md:h-full xl:h-[261px]'
              >
                <Image
